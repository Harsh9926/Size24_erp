...
/* ── Wallet History Modal (reusable for any shop) ─────────────────── */
const WalletHistoryModal = ({ shopId, shopName, onClose }) => {
    ...
    const EmptyState = ({ filter }) => (
        <div className="text-center py-10">
            ...
            {filter !== 'all' && <p className="text-xs text-gray-300 mt-1">Try <button onClick={() => { setHistFilter('all'); fetchHistory('all','',''); }} className="underline text-teal-500 font-semibold">All</button>.</p>}
        </div>
    );
            ...
            <div className="w-full max-w-2xl max-h-[90vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden"
                ...
                {viewMode === 'history' && (<>
                    ...
                    <div className="flex-1 overflow-y-auto px-6 py-4">
                        ...
                        : txns.length === 0 ? <EmptyState filter={histFilter} />
                    ...
                    <div className="flex-1 overflow-y-auto px-6 py-4">
                        ...
                        {!loading && !error && txns.length > 0 && (
                            <div className="space-y-2">
                                <p className="text-[10px] font-semibold uppercase tracking-wide mb-2" style={{ color:'var(--text-secondary)' }}>{txns.length} transaction{txns.length !== 1 ? 's':''}</p>
...
const AdminManagerFundsPage = () => {
    ...
    return (<>
                ...
                <>
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                            onClick={async () => {
                                try {
                                    const r = await api.post('/manager-transfers/sync-store-wallets');
                                    showToast('success', `Wallets synced — ${r.data.shops_changed} shop(s) updated.`);
                                    fetchAll();
                                } catch (err) {
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4 mb-8">
                        {storeWallets.map(sw => (
                            <div key={sw.shop_id}
                                ...
                                <button
                                    onClick={() => setHistoryShop({ id: sw.shop_id, name: sw.shop_name })}
        ...
        {/* Wallet History Modal — opens when admin clicks a store wallet */}
        {historyShop && (
            <WalletHistoryModal
