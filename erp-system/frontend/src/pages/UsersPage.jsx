...
import { UserPlus, Store, CheckCircle, XCircle, Clock } from 'lucide-react';
...
const UsersPage = () => {
    ...
    const handleCreate = async (e) => {
        e.preventDefault();
        try { await api.post('/users', form); setForm({ name: '', mobile: '', password: '', role: 'shop_user' }); loadAll(); notify('User created and approved automatically!'); }
        catch (err) { notify(err.response?.data?.error || 'Error creating user', 'error'); }
    };

    const handleAssign = async (e) => {
        e.preventDefault();
        try { await api.put(`/users/${assignForm.userId}/assign-shop`, { shopId: assignForm.shopId }); loadAll(); notify('Shop assigned!'); }
    ...
    return (
        <Layout title="User Management">
            {msg.text && (
                <div className={`mb-4 p-3 text-sm rounded-lg border ${msg.type === 'error' ? 'bg-red-50 border-red-200 text-red-700' : msg.type === 'warn' ? 'bg-yellow-50 border-yellow-200 text-yellow-700' : 'bg-green-50 border-green-200 text-green-700'}`}>
                    {msg.text}
                </div>
            )}
                        ...
                        {pendingUsers.map(u => (
                            <div key={u.id} className="flex items-center justify-between bg-white rounded-lg px-4 py-3 border border-amber-100 shadow-sm">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 font-mono">{u.mobile} &middot; <span className={`font-semibold ${roleColors[u.role]?.replace('bg-', 'text-').split(' ')[0]}`}>{u.role}</span></p>
                                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                    <h3 className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2"><UserPlus className="h-4 w-4 text-indigo-500" /> Create New User</h3>
                    <form onSubmit={handleCreate} className="space-y-4">
                        ...
                        <p className="text-xs text-gray-400">Users created here are auto-approved. Signup requests need manual approval.</p>
                ...
                {/* Assign Shop */}
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                    <h3 className="text-base font-bold text-gray-800 mb-4 flex items-center gap-2"><Store className="h-4 w-4 text-teal-500" /> Assign Shop to User</h3>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-100">
                        <thead className="bg-gray-50">
                            <tr>{['Name', 'Mobile', 'Role', 'Shop', 'Status'].map(h => <th key={h} className="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{h}</th>)}</tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {users.map(u => (
                                <tr key={u.id} className={`hover:bg-gray-50 ${!u.is_approved ? 'bg-amber-50/40' : ''}`}>
                                    ...
                                    <td className="px-5 py-3"><span className={`px-2 py-0.5 text-xs font-bold rounded-full ${roleColors[u.role] || 'bg-gray-100 text-gray-600'}`}>{u.role?.replace('_', ' ')}</span></td>
                            ...
                            {users.length === 0 && <tr><td colSpan="5" className="text-center py-10 text-gray-400">No users yet</td></tr>}
                        </tbody>
                    </table>
                </div>
            </div>
